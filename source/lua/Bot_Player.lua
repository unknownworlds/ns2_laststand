...
function BotPlayer:GetPlayerOrder()
    local order = nil
...
function BotPlayer:GivePlayerOrder(orderType, targetId, targetOrigin, orientation, clearExisting, insertFirst, giver)
    local player = self:GetPlayer()
...
function BotPlayer:GetPlayerHasOrder()
    local player = self:GetPlayer()
...
function BotPlayer:ChooseOrder()
    ...
    local player = self:GetPlayer()
    local order = self:GetPlayerOrder()
...
function BotPlayer:UpdateOrderVariables()
    ...
    if self:GetPlayerHasOrder() then
...
function BotPlayer:AttackVisibleTarget()
    ...
    if not self.timeLastTargetCheck or (Shared.GetTime() - self.timeLastTargetCheck > 2) then
        ...
        if nearestTarget then
            ...
            self:GivePlayerOrder(kTechId.Attack, nearestTarget:GetId(), nearestTarget:GetEngagementPoint(), nil, true, true)
...
function BotPlayer:ChooseRandomDestination(move)
    ...
    if ents:GetSize() > 0 then 
        ...
        self:GivePlayerOrder(kTechId.Move, 0, destination:GetEngagementPoint(), nil, true, true)
    ...
    if order ~= nil and (order:GetType() == kTechId.Attack) then
        ...
        if target then
            ...
            if activeWeapon and attackDist and (distToTarget < attackDist) then
                ...
                local trace = Shared.TraceRay(player:GetEyePos(), target:GetModelOrigin(), CollisionRep.LOS, PhysicsMask.AllButPCs, filter)
...
function BotPlayer:MoveToPoint(toPoint, move)
    ...
    move.yaw   = GetYawFromVector(direction) - player:GetBaseViewAngles().yaw
...
function BotPlayer:GetEngagementPointOverride()
