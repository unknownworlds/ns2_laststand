...
function Player:OnInitialized()
    ...
    if Server then
        ...
        InitViewModel(self)
        // Only give weapons when playing and not alien
...
local function AttemptToUse(self, timePassed)
    ...
    if entity then
        ...
        local canDoStuff = self:GetGameStarted()
        if self:GetTeamNumber() == kMarineTeamType then
            canDoStuff = GetGamerules():GetIsMarinePrepTime()
        end

        if not canDoStuff and not (entity.GetUseAllowedBeforeGameStart and entity:GetUseAllowedBeforeGameStart()) then
            if Client then
                ChatUI_AddSystemMessage("Game not started - can not use things.")
            end
            return false
        end
