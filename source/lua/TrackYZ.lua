...
local function FindConnectionPath(src, dst)
    ...
    local mask = CreateMaskExcludingGroups(PhysicsGroup.BigStructuresGroup, PhysicsGroup.PlayerControllersGroup, PhysicsGroup.PlayerGroup)
    ...
    for i=1,maxIterations do
        ...
        local trace = Shared.TraceRay(src, dst, CollisionRep.Move, mask)
        ...
        trace = Shared.TraceRay( endPoint, upPoint, CollisionRep.Move, mask )
...
function TrackYZ:FindPath(lastP, prevP, p)
    ...
    if v2:GetLength() < 1.2 then
        ...
        if dist < 0.05 then
           ...
           local trace = Shared.TraceRay(lastP, p, CollisionRep.Move, PhysicsMask.Bullets, TrackYZ.kFilter)
...
function TrackFinder:FindTrackTerminal(p, xzAxis)
    ...
    for _,y in ipairs({0,1,2,-1,-2}) do
        ...
        if trace.fraction > 0 then
            ...
            local thinTrace = Shared.TraceRay(endPoint, p, CollisionRep.Move, self.mask, self.filter)
...
function TrackFinder:TerminalTrace(p, xzAxis, xz, y)
    ...
    local trace = Shared.TraceBox(self.wedgeExtents, startP, p, CollisionRep.Move, self.mask, self.filter)
...
function TrackFinder:FloodFillPath()
    ...
    while self.planHeap.size > 0 and self.traceCount < maxTraces do
        ...
        if dist < minDistance then
            local endTrace = Shared.TraceRay(point, self.trackEnd, CollisionRep.Move, self.mask, self.filter)
...
function FFEntry:TraceDir(dir, len)
    ...
    local trace = Shared.TraceBox(self.finder.wedgeExtents, point, targetP, CollisionRep.Move, self.finder.mask, self.finder.filter)
...
function GroundTracker:DropPoint(ohp, lastDp)
    ...
    local trace = Shared.TraceBox(tf.wedgeExtents, ohp, ohp - Vector.yAxis * 10, CollisionRep.Move, tf.mask, tf.filter)
    local ep = trace.endPoint
    local losTrace = Shared.TraceRay(lastDp, ep, CollisionRep.Move, tf.mask, tf.filter)
    if TrackYZ.kTrace then
