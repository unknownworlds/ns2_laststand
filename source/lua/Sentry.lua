...
function Sentry:GetCanDigest(player)
    return self.deployed and player:GetIsAlive() and player:GetTeamNumber() == self:GetTeamNumber()
end

function Sentry:GetCanBeUsed(player, useSuccessTable)
    useSuccessTable.useSuccess = self:GetCanDigest(player)
end

function Sentry:OnDigested()

    DestroyEntity(self)

    // drop a pickup sentry
    local ent = CreateEntityForTeam( kTechId.DropSentry, self:GetOrigin(), self:GetTeamNumber(), nil )
    if ent.Dropped then
        ent:Dropped(nil)
    end
