...
function Player:UpdateScreenEffects(deltaTime)
    ...
    if Player.screenEffects.phase then
        ...
        if self.timeOfLastPhase and self.clientTimeOfLastPhase ~= self.timeOfLastPhase and (Shared.GetTime() - self.timeOfLastPhase <= kPhaseEffectActiveTime) then
...
function Player:GetWeaponClip()
    ...
    if weapon ~= nil then
        ...
        elseif weapon:isa("LSBuildSentry") then
            return 1
        end
    end
...
function PlayerUI_GetDamageIndicators()
    ...
    local player = Client.GetLocalPlayer()
    if player and player.damageIndicators ~= nil then
