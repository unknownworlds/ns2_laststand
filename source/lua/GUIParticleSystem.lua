...
local function GetFreeParticle(self)
...
function GUIParticleSystem:_EmitParticle(emitter)
    ...
    if table.count(emitter.particleTypesToEmit) > 0 then
        ...
        local numParticleTypesToEmit = #emitter.particleTypesToEmit
        assert(numParticleTypesToEmit > 0)
        
        local randomIndex = math.random(1, numParticleTypesToEmit)
        local particleTypeName = emitter.particleTypesToEmit[randomIndex]
        
        local randomParticleType = self.particleTypes[particleTypeName]
        assert(randomParticleType ~= nil)
        
        local freeParticle = GetFreeParticle(self)
        ...
        for funcName, values in pairs(randomParticleType) do
            ...
            local randomValueIndex = math.random(1, numValues)
            
            local randomValue = values[randomValueIndex]
            local itemOperation = freeParticle.Item[funcName]
            
            assert(type(itemOperation) == "function")
            assert(randomValue ~= nil)
