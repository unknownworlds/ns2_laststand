...
local networkVars =
{
    ...
    realFuel = "float(0.0 to 1.0 by 0.01)",
...
function JetpackMarine:GetRealFuel()
    return self.realFuel
end
...
function JetpackMarine:BackToNormalMarine()
    ...
    local normalMarine = self:Replace(Marine.kMapName, self:GetTeamNumber(), true, Vector(self:GetOrigin()))
    
    normalMarine:SetActiveWeapon(activeWeaponMapName)
    normalMarine:SetHealth(health)
...
function JetpackMarine:HandleButtons(input)
    ...
    // LS - let players give up jetpack
    /* Do not allow dropping jetpacks
    if bit.band( input.commands, Move.Drop ) ~= 0 then
        ...
        if weaponTarget then
            ...
            // let normal marine logic handle it
        
        else

            if Server then

                self:BackToNormalMarine()

                // Drop the jetpack pickup
                local pack = CreateEntityForTeam( kTechId.DropJetpack,
                        GetGroundAtPosition( self:GetOrigin(), nil, PhysicsMask.AllButPCs ),
                        self:GetTeamNumber(), nil )

            end

            // remove the command so the marine doesnt drop their weapon
            input.commands = RemoveMoveCommand( input.commands, Move.Drop )
        end
...
function JetpackMarine:GiveExo(spawnPoint, layout, oldArmor)

    // before we transform to exo, drop a jetpack
    local pack = CreateEntityForTeam( kTechId.DropJetpack,
            GetGroundAtPosition( self:GetOrigin(), nil, PhysicsMask.AllButPCs ),
            self:GetTeamNumber(), nil )

    Marine.GiveExo( self, spawnPoint, layout, oldArmor )
...
function JetpackMarine:OnProcessMove(input)
    ...
    if self.jetpacking then
        self.realFuel = self.realFuel - 1.0/30.0 * dt
    end

    if self.realFuel <= 0.0 then
        self:BackToNormalMarine()
    end
