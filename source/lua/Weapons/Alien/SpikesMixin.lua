...
function SpikesMixin:OnSecondaryAttack(player)
    ...
    if player:GetHasUpgrade(kTechId.Spikes) and player:GetEnergy() >= self:GetSecondaryEnergyCost(player) and not self.primaryAttacking then
...
function SpikesMixin:OnSecondaryAttackEnd(player)
    ...
    self.secondaryAttacking = falseg
    self.shootingSpikes = false
