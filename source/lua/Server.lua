...
local function OnMapPostLoad()
    ...
    // Create the global game controller entity
    gGameState = CreateEntity( "ns2_gamerules" )
...
//----------------------------------------
//  game rules singleton
//----------------------------------------
gGameState = gGameState or nil

function GetHasGameRules()
    return gGameState ~= nil
end

function SetGamerules(gamerules)

    gGameState = gamerules
...
//----------------------------------------
Event.Hook("Console_ls_start", function()
        GetGamerules():SetGameState(kGameState.PreGame)
        end)
